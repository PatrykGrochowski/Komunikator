...
namespace InstantMessengerServer
{
    public class Client
    {
        ...
        void SetupConn()  
        {
            try
            {
                Console.WriteLine("[{0}] Nowe połaczenie!", DateTime.Now);
                netStream = client.GetStream();
                ssl = new SslStream(netStream, false);
                ssl.AuthenticateAsServer(prog.cert, false, SslProtocols.Tls, true);
                Console.WriteLine("[{0}] Połączenie zatwierdzone!", DateTime.Now);
                        ...
                        if (password.Length < 10)  
                        {
                            if (logMode == IM_Register)  
                            {
                                if (!prog.users.ContainsKey(userName))  
                                {
                                    ...
                                    bw.Write(IM_OK);
                                    bw.Flush();
                                    Console.WriteLine("[{0}] ({1}) Rejestracja nowego użytkownika", DateTime.Now, userName);
                                    prog.SaveUsers();
                                    Receiver(); 
                                }
                                else
        ...
        void CloseConn() 
        {
            try
            {
                ...
                netStream.Close();
                client.Close();
                Console.WriteLine("[{0}] Zakończenie połączenia!", DateTime.Now);
            }
            catch { }
        }
        void Receiver()  
        {
            Console.WriteLine("[{0}] ({1}) Użytkownik zalogowany do", DateTime.Now, userInfo.UserName);
                    ...
                    else if (type == IM_Send)
                    {
                        ...
                        if (prog.users.TryGetValue(to, out recipient))
                        {
                            ...
                            if (recipient.LoggedIn)
                            {
                                ...
                                Console.WriteLine("[{0}] ({1} -> {2}) Wiadomość przesłana!", DateTime.Now, userInfo.UserName, recipient.UserName);
                            }
                        }
                    }
                }
            ...
            userInfo.LoggedIn = false;
            Console.WriteLine("[{0}] ({1}) Użytkownik wylogowany!", DateTime.Now, userInfo.UserName);
        }
